...
+ (UIButton *)creatBtnWithTitle:(NSString *)title withLabelFont:(NSInteger)font andBackGroundColor:(UIColor *)backGroundColor WithTarget:(id)delegate andDoneAtcion:(SEL)doneAtcion andSuperView:(UIView *)superView {
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    
    if (backGroundColor) {
        [btn setBackgroundImage:[UIImage imageWithColor:[backGroundColor colorWithAlphaComponent:.7]] forState:UIControlStateNormal];
        [btn setBackgroundImage:[UIImage imageWithColor:[backGroundColor colorWithAlphaComponent:.6]] forState:UIControlStateHighlighted];
    }
    [superView addSubview:btn];
    [btn addTarget:delegate action:doneAtcion forControlEvents:UIControlEventTouchUpInside];
...
+ (UIButton *)creatBtnWithTitle:(NSString *)title andBorderColor:(UIColor *)borderColor WithTarget:(id)delegate andDoneAtcion:(SEL)doneAtcion andSuperView:(UIView *)superView {
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    ...
    btn.layer.shadowOffset = CGSizeMake(3,3);//shadowOffset阴影偏移,x向右偏移4，y向下偏移4，默认(0, -3),这个跟shadowRadius配合使用
    btn.layer.shadowOpacity = .5;//阴影透明度，默认0
...
//    btn.layer.masksToBounds = YES;
    [btn setTitle:title forState:UIControlStateNormal];
    btn.titleLabel.font = [UIFont systemFontOfSize:k14];
